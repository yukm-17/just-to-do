...
import {
  ...
  IconButton, Separator, Center, HStack, Button,
...
export default function App() {
  ...
  const {
    ...
    addTodo, addChild, toggleTodo, deleteTodo, editTodo,
          ...
          {view === 'today' && (
            <Box bg="bg.panel" borderRadius="2xl" p={{ base: '4', md: '5' }} shadow="md">
              {todos.length === 0 ? (
                <Center py={10}>
                  <VStack gap={2}>
                    <Text fontSize="sm" color="fg.muted">할 일이 없어요</Text>
                    <Button size="sm" colorPalette="purple" variant="ghost" onClick={() => setView('all')}>
                      전체 할 일에서 추가하기 →
                    </Button>
                  </VStack>
                </Center>
